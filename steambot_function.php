...
function send($token = '', $json, $accountid)
{
    ...
    $params = [
        ...
        //身份验证用
        'serverid' => '1',
        'partner' => toCommunityID($accountid),
        //目标steamID
        'tradeoffermessage' => time(),
        //交易留言
        'json_tradeoffer' => $json,
        //交易传参,type:json
        'trade_offer_create_params' => empty($token) ? "{}" : json_encode(['trade_offer_access_token' => $token]),
    ];
...
{
    //模拟登录,获取cookie[username:用户名,password:密码,twofa:二步验证码(无则不需填写)][返回JSON]
    ...
    $key = ['modulus' => new Math_BigInteger($json['publickey_mod'], 16), 'publicExponent' => new Math_BigInteger($json['publickey_exp'], 16)];
    $rsa->loadKey($key, CRYPT_RSA_PUBLIC_FORMAT_RAW);
    $encryptedPassword = base64_encode($rsa->encrypt($password));
    $params = ['username' => $username, 'password' => $encryptedPassword, 'twofactorcode' => $twofa, 'captchagid' => '-1', 'captcha_text' => '', 'emailsteamid' => '', 'emailauth' => '', 'rsatimestamp' => $json['timestamp'], 'remember_login' => 'false'];
...
function getoffer($key, $tradeOfferId)
{
    //获取交易细节[key:API秘钥,tradeOfferId:交易ID][返回JSON]
    return apirequest($key, array('method' => 'GetTradeOffer/v1', 'params' => array('tradeofferid' => $tradeOfferId, 'language' => 'CN')));
}
function canceloffer($key, $tradeOfferId)
{
    //取消交易[key:API秘钥,tradeOfferId:交易ID][返回BOOLEAN]
    return apirequest($key, array('method' => 'CancelTradeOffer/v1', 'params' => array('tradeofferid' => $tradeOfferId)));
}
function declineoffer($key, $tradeOfferId)
{
    //拒绝交易[key:API秘钥,tradeOfferId:交易ID][返回BOOLEAN]
    return apirequest($key, array('method' => 'DeclineTradeOffer/v1', 'param' => array('tradeofferid' => $tradeOfferId), 'post' => 1));
}
function acceptoffer($option, $partner)
{
    //接受交易[option:交易ID,partner:交易者ID(发起这次交易的那个)][返回BOOLEAN]
    $form = array('sessionid' => getSession(), 'serverid' => 1, 'tradeofferid' => $option, 'partner' => $partner);
...
{
    //发起API类请求[key:API秘钥,option:请求参数][返回JSON]
...
{
    //获取用户的游戏列表[nickname:玩家昵称][返回JSON]
...
{
    //获取用户库存[steamid:用户64位ID,gameid:游戏ID][返回JSON]
